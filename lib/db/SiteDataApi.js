...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  if (post) {
    ...
    try {
      ...
      // 兜底：processPostData 中途抛错会跳过 prev/next/recommendPosts 的赋值，
      // 给前端组件提供安全默认值，避免渲染期 .map/.length 崩溃导致整页预渲染失败(build exit 1)
      props.prev = props.prev ?? null
      props.next = props.next ?? null
      props.recommendPosts = props.recommendPosts ?? []
    }
